...
//Konstanten
const SQL_INSERT='INSERT INTO Patient (`Nachname` , `Name` , `Geburtsdatum` , `Telefonnummer` , `Adresse_Nachname` , `Adresse_Vorname` , `Adresse_Straße` , `Adresse_PLZ` , `Adresse_Ort` ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
const SQL_UPDATE='UPDATE Patient SET `Nachname` =?, `Name` =?, `Geburtsdatum` =?, `Telefonnummer` =?, `Adresse_Nachname` =?, `Adresse_Vorname` =?, `Adresse_Straße` =?, `Adresse_PLZ` =?, `Adresse_Ort` =? where id=?';
const SQL_SELECT_PK='SELECT Patient.`c_ts` as `c_ts`, Patient.`m_ts` as `m_ts`, Patient.`id` as `id`, `Patient`.`Nachname` as `Nachname`, `Patient`.`Name` as `Name`, `Patient`.`Geburtsdatum` as `Geburtsdatum`, `Patient`.`Telefonnummer` as `Telefonnummer`, `Patient`.`Adresse_Nachname` as `Adresse_Nachname`, `Patient`.`Adresse_Vorname` as `Adresse_Vorname`, `Patient`.`Adresse_Straße` as `Adresse_Straße`, `Patient`.`Adresse_PLZ` as `Adresse_PLZ`, `Patient`.`Adresse_Ort` as `Adresse_Ort` from Patient where Patient.`id` = ?';
...
const SQL_SELECT_Impftermin_a='select Patient.id as id, Patient.Nachname as Nachname, Patient.Name as Name, Patient.Geburtsdatum as Geburtsdatum, Patient.Telefonnummer as Telefonnummer, Patient.Adresse_Nachname as Adresse_Nachname, Patient.Adresse_Vorname as Adresse_Vorname, Patient.Adresse_Straße as Adresse_Straße, Patient.Adresse_PLZ as Adresse_PLZ, Patient.Adresse_Ort as Adresse_Ort from Patient where Patient.id = ?';
}
