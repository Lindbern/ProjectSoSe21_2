...
//Konstanten
const SQL_INSERT='INSERT INTO Impftermin (_Patient, _Arzt, `Termin` , `Aussage` , `Ausführung` , `Impfbericht` ) VALUES (?, ?, ?, ?, ?, ?)';
const SQL_UPDATE='UPDATE Impftermin SET _Patient=?, _Arzt=?, `Termin` =?, `Aussage` =?, `Ausführung` =?, `Impfbericht` =? where id=?';
const SQL_SELECT_PK='SELECT Impftermin.`c_ts` as `c_ts`, Impftermin.`m_ts` as `m_ts`, Impftermin.`id` as `id`, Impftermin._Patient as _Patient, Impftermin._Arzt as _Arzt, `Impftermin`.`Termin` as `Termin`, `Impftermin`.`Aussage` as `Aussage`, `Impftermin`.`Ausführung` as `Ausführung`, `Impftermin`.`Impfbericht` as `Impfbericht` from Impftermin where Impftermin.`id` = ?';
const SQL_SELECT_ALL='SELECT Impftermin.`c_ts` as `c_ts`, Impftermin.`m_ts` as `m_ts`, Impftermin.`id` as `id`, Impftermin._Patient as _Patient, Impftermin._Arzt as _Arzt, `Impftermin`.`Termin` as `Termin`, `Impftermin`.`Aussage` as `Aussage`, `Impftermin`.`Ausführung` as `Ausführung`, `Impftermin`.`Impfbericht` as `Impfbericht` from Impftermin';
const SQL_SELECT_IGNORE_DERIVED='SELECT DISTINCT Impftermin.`c_ts` as `c_ts`, Impftermin.`m_ts` as `m_ts`, Impftermin.`id` as `id`, Impftermin._Patient as _Patient, Impftermin._Arzt as _Arzt, `Impftermin`.`Termin` as `Termin`, `Impftermin`.`Aussage` as `Aussage`, `Impftermin`.`Ausführung` as `Ausführung`, `Impftermin`.`Impfbericht` as `Impfbericht` from Impftermin';
const SQL_DELETE='DELETE FROM Impftermin WHERE id=?';
const SQL_PRIMARY='id';

const SQL_SELECT_Patient='select Impftermin.id as id, Impftermin.Termin as Termin, Impftermin.Aussage as Aussage, Impftermin.Ausführung as Ausführung, Impftermin.Impfbericht as Impfbericht from Impftermin where Impftermin._Patient = ?';
